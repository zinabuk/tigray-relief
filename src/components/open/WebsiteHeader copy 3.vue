<template>
  <header class="w-full sticky top-0 z-50 bg-gray-900 text-white shadow-lg">
    ...
    <div class="w-full bg-gradient-to-r text-green-900 from-white to-white flex justify-between items-center px-4 py-2 md:px-8">
    ...
    <!-- Desktop Navigation -->
    <nav class="hidden md:flex w-full px-8 py-4 items-center justify-between bg-[#148C4E]">
...
watch(
  () => route.name,
  (newRoute) => {
    if (newRoute === 'services' || newRoute === 'service-detail') {
      fetchServices();
    }
    if (newRoute === 'departments' || newRoute === 'department-detail') {
      fetchDepartments();
    }
  }
);
  ...
  try {
    ...
    if (response.success) {
      departments.value = response.data.map((item) => ({
        ...item,
        departmentName: JSON.parse(item.departmentTitle),
        departmentDescription: JSON.parse(item.departmentDescription),
      }));
    }
    // alert(departments.value.length);
  } catch (error) {
    alert('error', error);
  }
};
