...
const tableHeaders = [
  { label: 'Service', field: 'interestedIn' },
  { label: 'First Name', field: 'firstName' },
  { label: 'Last Name', field: 'lastName' },
  { label: 'Email', field: 'email' },
  { label: 'Phone Number', field: 'phoneNumber' },
  { label: 'Address', field: 'description' },
]
const donations = ref([])
const actions = [
  {
...
async function fetchDonations() {
  const response = await ApiService.get('/users/volunteers')
...
const updateDonation = async () => {
  ...
  try {
    const response = await ApiService.patch(
      '/users/volunteers/' + editForm.value._id,
      editForm.value
    )
...
async function deleteDonation(donation) {
  ...
  if (accept) {
    const response = await ApiService.delete(`/users/volunteers/${donation._id}`)
    if (response.success) {
      fetchDonations()
    }
  }
